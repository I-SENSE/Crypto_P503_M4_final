...
{ // Multiprecision multiplication, c = a*b mod p.
    ...
    //mp_mul(ma, mb, temp, NWORDS_FIELD);
		multi_testing(ma, mb, temp, NWORDS_FIELD);
    rdc_mont(temp, mc);
}
...
{ // Multiprecision squaring, c = a^2 mod p.
   ...
   // mp_mul(ma, ma, temp, NWORDS_FIELD);
		multi_testing(ma, ma, temp, NWORDS_FIELD);
    rdc_mont(temp, mc);
}
...
static digit_t mp_subfast(const digit_t* a, const digit_t* b, digit_t* c)
...
{ // GF(p^2) multiplication using Montgomery arithmetic, c = a*b in GF(p^2).
  ...
  // Output: c = c0+c1*i, where c0, c1 are in [0, 2*p-1] 
    ...
    //mp_mul(a[0], b[0], tt1, NWORDS_FIELD);           // tt1 = a0*b0
		multi_testing(a[0], b[0], tt1, NWORDS_FIELD);  
    //mp_mul(a[1], b[1], tt2, NWORDS_FIELD);           // tt2 = a1*b1
		multi_testing(a[1], b[1], tt2, NWORDS_FIELD);   
    //mp_mul(t1, t2, tt3, NWORDS_FIELD);               // tt3 = (a0+a1)*(b0+b1)
		multi_testing(t1, t2, tt3, NWORDS_FIELD);  
